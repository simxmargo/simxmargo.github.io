// Pure, dependency-free HTML <meta>/<title>/favicon extractors for the admin
// "Add brand from URL" feature (Phase 6). Node/Next-safe — this is the twin of
// the Deno-only supabase/functions/_shared/scrape.ts helpers, kept here so it's
// type-checked by tsc and runs inside the /api/admin/scrape-meta Route Handler
// (the supabase/ dir is excluded from the app tsconfig). No network or DB here —
// the Route Handler owns all I/O; these stay pure so they're trivially testable.
...
// IP literal (the Route Handler calls it on both). Hostname-level checks can't
// stop DNS rebinding on their own — the Route Handler also resolves the host and
// re-runs this on each returned IP; full protection still needs egress firewalling.
